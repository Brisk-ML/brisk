...
def sample_df_with_groups():
    return pd.DataFrame({
        ...
        "group_id": [1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6, 7, 7],
...
class TestDataManagerIntegration:
    ...
    def patch_slow_methods(self):
        """Patch time-consuming DataSplitInfo methods for all tests in this class."""
        with mock.patch.object(DataSplitInfo, "set_services", return_value=None), \
             mock.patch.object(DataSplitInfo, "evaluate_data_split", return_value=None):
            yield
    ...
    def test_split_different_keys_not_cached(self, tmp_path, csv_file, mock_services):
        with project.ProjectRootContext(tmp_path):
            manager = data_manager.DataManager()
            manager.set_services(mock_services)
